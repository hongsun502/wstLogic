...
for i in range(1,10000):
    ...
    state2 = state1 + random.randint(1, 10 - state1)
    f.write('{care:Parkinson gps:description (\n')
    f.write('     {?patient care:state care:state_' + str(state1) +'.}\n')
    f.write('     {?patient gps:medication medication:Medication_'+ str(random.randint(1,100)) +'.}\n')
    f.write('     {?patient care:state care:state_' + str(state2) +'.}\n')

    f.write('     action:take_pill_Medication_'+ str(random.randint(1,100)) + '\n')
